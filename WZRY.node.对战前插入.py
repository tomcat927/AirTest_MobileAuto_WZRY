...
#self.标准触摸对战=True
#组队时顺便完成人机检测
#if self.组队模式: self.标准触摸对战 = True
#自动分配对战位置
weekday=datetime.now(eastern_eight_tz).weekday()
#if self.jinristep > 10 and weekday > 3: self.触摸对战=False
if self.组队模式 and hour <= self.限时组队时间 and self.jinristep < 4: self.标准触摸对战 = True
#if self.组队模式 and hour >= self.限时组队时间 -2 : self.标准触摸对战 = True
#
字典位置文件=[]
...
TimeECHO(self.prefix+f"本步{self.runstep}使用字典文件{字典位置文件[此步位置文件]}")
